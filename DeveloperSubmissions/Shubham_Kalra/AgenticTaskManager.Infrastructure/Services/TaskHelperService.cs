...
public class TaskHelperService : IDisposable
{
    ...
    public async Task<TaskStatistics> CalculateTaskStatisticsAsync()
    {
        try
        {
            ...
            var completedTasks = taskList.Count(t => t.Status == TaskStatus.Completed);
    ...
    /// <returns>Tasks with the specified status, ordered by creation date descending.</returns>
    public async Task<IEnumerable<TaskItem>> GetTasksByStatusAsync(TaskStatus status)
    {
        try
        {
            _logger.LogDebug("Retrieving tasks with status {Status}", status);
            ...
            _logger.LogInformation("Found {TaskCount} tasks with status {Status}", 
                filteredTasks.Count, status);
    ...
    public async Task<IEnumerable<TaskItem>> GetOverdueTasksAsync()
    {
        try
        {
            ...
            var overdueTasks = tasks.Where(t => t.DueDate < currentDate && 
    ...
    /// <summary>
    /// Updates the status for multiple tasks in a single operation.
    /// </summary>
    ...
    public async Task<bool> BulkUpdateTaskStatusAsync(IEnumerable<Guid> taskIds, TaskStatus newStatus)
    {
        ...
        try
        {
            _logger.LogInformation("Starting bulk status update for {TaskCount} tasks to status {Status}", 
                taskIds.Count(), newStatus);
            ...
            foreach (var taskId in taskIdList)
            {
                ...
                // Note: TaskItem doesn't have UpdatedAt property in current model
            }

            _logger.LogInformation("Successfully bulk updated {TaskCount} tasks with status {Status}", 
                taskIdList.Count, newStatus);
            
            return true;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error during bulk status update for tasks: {TaskIds}", 
                string.Join(", ", taskIds));
            return false;
        }
    }
    ...
    public async Task<int> ArchiveCompletedTasksAsync()
    {
        try
        {
            ...
            {
                // Note: Current TaskStatus enum doesn't have Archived status
                // Using Failed as a substitute or add Archived to the enum
                task.Status = TaskStatus.Failed; // Placeholder
                archivedCount++;
            }
    ...
    public async Task ExportTasksToFileAsync(IEnumerable<TaskItem> tasks, string filePath)
    {
        ...
        try
        {
            ...
            // Write CSV header
            await writer.WriteLineAsync("Id,Title,Description,Status,CreatedById,AssignedToId,DueDate,CreatedAt");
